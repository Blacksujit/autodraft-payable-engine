...
from autodraft.structure import clean_numeric, amount_value, _has_decimal, _numeric_word, _build_table, _cluster_centers, _is_summary_label
...
print('Run rows:', [i for i in range(with_header, best_end + 1)])
...
num_words = [w for w in run for ln in r for w in ln.words if _numeric_word(w) is not None]
