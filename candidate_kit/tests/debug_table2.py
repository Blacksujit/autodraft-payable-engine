...
for r_i, r in enumerate(run):
    ...
    # Check desc edge computation (new logic)
    if key != best_start and any(i >= 0 and i in [1, 2] for i in row_map):
        if -1 in row_map:
            desc_xs = []
            for bx, tx in row_map[-1]:
                stripped = tx.strip()
                if stripped.isdigit():
                    continue
            ...
            if desc_xs:
                print('Row', key, 'desc_xs=', desc_xs, 'min=', min(desc_xs))
"