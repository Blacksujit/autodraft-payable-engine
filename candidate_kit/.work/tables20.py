...
for name in ("INV-10","INV-13","INV-34","INV-16","INV-26"):
    for i in (1,2,3):
        ...
        for r in sorted(t.rows):
            print("   r",r,"DESCR=",repr(t.cell(r,-1))[:55], "|", " | ".join(f"c{c.index}={t.cell(r,c.index)!r}" for c in t.columns if t.cell(r,c.index)))
